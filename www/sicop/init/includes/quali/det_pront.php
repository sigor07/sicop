...
$motivo_pag = 'QUALIFICATIVA DE DETENTO - PRONTUÁRIO';
...
$query = "SELECT
            ...
            `detentos`.`resid_det`,
            `detentos`.`conduta_ant`,
            DATE_FORMAT(`detentos`.`data_reab`, '%d/%m/%Y') AS data_reab_f,
            `detentos`.`motivo_prisao`,
            `detentos`.`n_passagem`,
            `tipoconduta`.`conduta`,
          ...
          FROM
            ...
            LEFT JOIN `tipoconduta` ON `detentos`.`conduta_ant` = `tipoconduta`.`idconduta`
          WHERE
...
$sit_process = $d_det['sit_proc'];
$font_in = '';
$font_out = '';
if ( empty( $sit_process ) ) {

    $font_in = '<font color="#FF0000">';
    $font_out = '</font>';
}

//$query_pena = "CALL `cal_pena`('$iddet')";
//$query_pena = $mysqli->query($query_pena);
...
<table class="detal_det_min">
    ...
    <tr>
        <td class="mid" colspan="2">Artigo: <?php echo $d_det['artigo'] ?></td>
        <td class="mid" colspan="2">Primário: <?php echo tratasn( $d_det['primario'] ) ?></td>
    </tr>
    <tr>
        <td class="mid" colspan="2"><?php echo $font_in; ?>Situação Processual:<?php echo $font_out; ?> <?php echo $d_det['sit_proc'] ?></td>
        <td class="mid" colspan="2">Condenação: <?php echo $pena; ?></td>
    </tr>
    <tr>
        <td class="mid" colspan="2">Conduta da unidade anterior: <?php echo $d_det['conduta'] ?></td>
        <td class="mid" colspan="2">Data da reabilitação: <?php echo $d_det['data_reab_f'] ?></td>
    </tr>
    <tr>
        <td class="great" colspan="4">Última residência: <?php echo $d_det['resid_det'] ?></td>
    </tr>
    <tr>
        <td class="mid" colspan="2">Situação atual: <b><span style="font-size: 10px;" class="<?php echo $det['css_class'];?>"><?php echo $det['sitat'] ?></span></b></td>
