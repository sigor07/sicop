...
$motivo_pag = 'QUALIFICATIVA DE DETENTO - COMPLETA';
...
<table class="detal_det_min">
    <tr>
        <td class="mid">Nome: <a href="<?php echo SICOP_ABS_PATH; ?>detento/detalhesdet.php?iddet=<?php echo $iddet; ?>" title="Clique aqui para abrir a qualificativa deste detento"><?php echo $d_det['nome_det']; ?></a></td>
        <td class="mini">Matrícula: <?php if ( !empty( $d_det['matricula'] ) ) echo formata_num( $d_det['matricula'] ) ?></td>
        <td class="mini" rowspan="7" align="center">
            <?php if ( $amplia ){ ?>
            ...
            <?php }; ?>
            <img src="<?php echo $foto_det ?>" alt=""  class="foto_det_m" />
            <?php if ( $amplia ){ ?></a><?php } ?>
        </td>
    </tr>
    <tr>
        <td class="mid">Vulgo(s): <?php echo $d_det['vulgo'] ?></td>
        <td class="mini">RG Civil: <?php echo formata_num( $d_det['rg_civil'] ) ?></td>
    </tr>
    <tr>
        <td class="mid">Data de Nascimento: <?php echo (empty( $d_det['nasc_det'] )) ? '' : $d_det['nasc_det'] . ' - '. $d_det['idade_det'] . ' anos'; ?></td>
    ...
    <tr>
        ...
        <td class="mini"><?php echo $fontautv ?>Pode receber visitas: <?php echo tratasn( $d_det['aut_visita'] ) . $fontautvf ?></td>
    </tr>
    <tr>
        <td class="mid">Mãe: <?php echo $d_det['mae_det'] ?></td>
        <td class="mini"><?php echo $fontauts ?>Pode receber sedex: <?php echo tratasn( $d_det['aut_sedex'] ) . $fontautsf ?></td>
    </tr>
    <tr>
        <td class="mid">Situação atual: <b><span style="font-size: 12px;" class="<?php echo $det['css_class'];?>"><?php echo $det['sitat'] ?></span></b></td>
        <td class="mini_rc"><?php echo empty( $d_det['raio'] ) ? '' : SICOP_RAIO . ': ' . $d_det['raio']; ?> <?php echo empty( $d_det['cela'] ) ? '' : ' - ' . SICOP_CELA . ': ' . $d_det['cela']; ?></td>
    </tr>
    <tr>
