...
?>
            ...
            <p class="descript_page">ALTERAR ATESTADO DE PERMANÊNCIA</p>

            <?php include 'quali/det_cad.php'; ?>

            <p class="table_leg">Alterar atestado</p>
            ...
            <form action="<?php echo SICOP_ABS_PATH; ?>send/sendapcc.php" method="post" name="sendapcc" id="sendapcc" onSubmit="return validaapcc();">
                ...
                $q_mov_in = "SELECT
                             ...
                             WHERE
                               ( `mov_det`.`cod_tipo_mov` = 1 OR `mov_det`.`cod_tipo_mov` = 3 ) AND `mov_det`.`cod_detento` = $iddet
                             ORDER BY
                               `mov_det`.`data_mov` ASC, `mov_det`.`data_add` ASC";
                ...
                ?>
                        <tr class="even">
                            ...
                            $check = '';
                            $q_mov_apcc = "SELECT `cod_movin` FROM `apcc_mov` WHERE `cod_apcc` = $idapcc AND `cod_movin` = $idmovin";

                            // executando a query
                            $q_mov_apcc = $model->query( $q_mov_apcc );

                            $cont_mov_apcc = $q_mov_apcc->num_rows;
                            if ( $cont_mov_apcc == 1 ) $check = 'checked="checked"';

                            ?>
                            <td align="center"><input name="idmovin[]" type="checkbox" class="mark_row" id="idmovin" value="<?php echo $idmovin ?>" <?php echo $check ?>/></td>
                        </tr>
                    <?php
                ...
                <div style="margin: 5px auto; text-align: center;">
                    <select name="conduta" class="CaixaTexto" id="conduta" onchange="mostraPDA_APCC()">
                        ...
                        <option value="<?php echo $d_cond['idconduta']; ?>" <?php echo $d_cond['idconduta'] == $d_apcc['cod_conduta'] ? 'selected="selected"' : '' ?> ><?php echo $d_cond['conduta']; ?></option>
                        <?php }; ?>
                    </select>
                    &nbsp;&nbsp;&nbsp;
                    <span id="f_pda">Número do PDA: <input name="pda" type="text" class="CaixaTexto" id="pda" onblur="upperMe(this); remacc(this);" onkeypress="return blockChars(event,6);" value="<?php echo!empty( $d_apcc['num_pda'] ) ? $d_apcc['num_pda'] : ''; ?>" size="10" maxlength="11" /></span>
                ...
                <div class="form_bts">
                    <input class="form_bt" name="atualizar" type="submit" value="Alterar" />
            ...
            if( $querysind ) $conts = $querysind->num_rows;
            ...
            // executando a query
            $q_apcc_old = $model->query( $q_apcc_old );
            ...
            if( $q_apcc_old ) $contapcc = $q_apcc_old->num_rows;

            if ( $contapcc < 1 ) { // se o número de ocorrências for menor do que 1, mostra a mensagem
                ...
                <table class="lista_busca">
                    ...
                    <?php while ( $d_apcc_old = $q_apcc_old->fetch_assoc() ) { ?>
                    <tr class="even_dk">
                        <td width="120" align="center"><a href="<?php echo SICOP_ABS_PATH; ?>cadastro/detalapcc.php?idapcc=<?php echo $d_apcc_old['idapcc']; ?>" title="Clique aqui para ver os detalhes deste APCC"><?php echo $d_apcc_old['numero_apcc'] . '/' . $d_apcc_old['ano'] ?></a></td>
                        <td width="80" align="center"><?php echo $d_apcc_old['data_add_f'] ?></td>
                    </tr>
                    <?php } // fim do while ?>
