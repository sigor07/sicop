...
$motivo_pag = 'BUSCA DE DELEGACIAS GSA';
...
if( !empty( $_GET['busca'] ) ) {
    ...
    switch( $ordpor ) {
        default:
        case 'dpa':
            $ordbusca = "`gsa_depol_nome` ASC";
            break;
        case 'dpd':
            $ordbusca = "`gsa_depol_nome` DESC";
            break;
        case 'coda':
            $ordbusca = "`gsa_depol_cod` ASC";
            break;
        case 'codd':
            $ordbusca = "`gsa_depol_cod` DESC";
            break;
    }
    ...
    $q_gsa = "SELECT
              ...
              FROM
                `gsa_depol`
              WHERE
                `gsa_depol_nome` LIKE '%$campo_gsa%' OR
                `gsa_depol_cod` LIKE '$campo_gsa%'
              ORDER BY
                $ordbusca";
    ...
    $mensagem = "[ BUSCA EFETUADA ]\nBusca de código de delegacias do GSA efetuada\n\n $valor_busca\n\n Página: $pag";
    salvaLog($mensagem);
...
$desc_pag = 'Pesquisar códigos das delegacias do GSA';
...
?>
            ...
            <p class="descript_page"> PESQUISAR DELEGACIAS DO GSA </p>


            <form action="<?php echo $_SERVER['PHP_SELF']?>" method="get" name="buscagsa" id="buscagsa">


                <p class="table_leg">Digite o CÓDIGO ou o NOME da delegacia:</p>
            ...
            <table class="lista_busca">
                <tr class="cab">
                    ...
                    <th class="local_gsa">Delegacia
                        <?php if ($ordpor == 'dpa') {?>
                        <img src="<?php echo SICOP_SYS_IMG_PATH; ?>s_asc_m.png" alt="" width="11" height="9" />
                        <?php } else { ?>
                        <a href="?<?php echo http_build_query( $q_string )?>&op=dpa" title="Ordenar pela delegacia crescente" ><img src="<?php echo SICOP_SYS_IMG_PATH; ?>s_asc.png" alt="Ordenar pela delegacia crescente" width="11" height="9" /></a>
                        <?php }; ?>
                        <?php if ($ordpor == 'dpd') {?>
                        <img src="<?php echo SICOP_SYS_IMG_PATH; ?>s_desc_m.png" alt="" width="11" height="9" />
                        <?php } else { ?>
                        <a href="?<?php echo http_build_query( $q_string )?>&op=dpd" title="Ordenar pela delegacia decrescente" ><img src="<?php echo SICOP_SYS_IMG_PATH; ?>s_desc.png" alt="Ordenar pela delegacia decrescente" width="11" height="9" /></a>
                        <?php }; ?>
                    </th>
                </tr>
                    <?php
                ...
                <tr class="even_gsa">
                    ...
                    <td class="cod_gsa"> <?php echo $d_gsa['gsa_depol_cod'];?> </td>
                    <td class="local_gsa"> <?php echo $d_gsa['gsa_depol_nome'];?> </td>
                </tr>
                    <?php } // fim do while ?>
            </table>
