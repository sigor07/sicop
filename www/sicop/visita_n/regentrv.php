...
if ( $n_rol < $n_rol_n ) {
    ...
    $msg = array();
    $msg['tipo']  = 'perm';
    $msg['entre_ch'] = 'REGISTRO DE ENTRADA DE VISITANTES';
    get_msg( $msg, 1 );
...
if ( empty( $iddet ) ){
    $mensagem = "<span class='desc_atencao'>*** ATENÇÃO ***</span> --> Tentativa de acesso direto à página de registro de entrada do rol de visitas.\n\n Página: $pag";
    salvaLog( $mensagem );
    echo msg_js( '', 1 );
    exit;
}
...
?>
            ...
            <p class="descript_page">REGISTRAR ENTRADA DE VISITANTES</p>

            <?php if ($imp_rol >= 1){?>
            <p class="link_common">
                <a href="#" onClick="javascript: ow('../print/rol_visit.php?iddet=<?php echo $iddet;?>', '600', '400'); return false" title="Imprimir o rol de visitas deste detento">Imprimir rol</a>
            </p>
            <?php } ?>

            <?php if ( $n_rol >= 3 ) {?><p class="link_common"><a href="alt_perm.php?iddet=<?php echo $iddet;?>" title="Alterar as permissões de acesso de visitantes e de recebimento de sedex deste detento">Alterar permissões</a></p><?php }?>
            ...
            <table class="lista_busca">
                    ...
                    <th class="tb_bt">&nbsp;</th>
                        ...
                        while( $dadosv = $queryvis->fetch_assoc() ) {
                            ...
                            if ( !$suspenso ) {
                                ...
                                $corfontv = '#000000';
                                $query_ent = "SELECT `idmov_visit`, `jumbo` FROM `visita_mov` WHERE `cod_visita` = $idvisita AND DATE(`data_in`) = DATE(NOW()) LIMIT 1";
                                ...
                                if( $cont_ent >= 1 ) {
                                    $entrou  = true;
                                    $d_jumbo = $query_ent->fetch_assoc(); // verifica se entrou com jumbo
                                    $jumbo   = $d_jumbo['jumbo'];
                                }
                            }
                ...
                <tr class="even" title="Situação do visitante: <?php echo $sit_v_atual; ?>">
                    ...
                    <td class="visit_nome"><a href="detalvisit.php?idvisit=<?php echo $dadosv['idvisita'] ?>&proced=1"><?php echo $dadosv['nome_visit'] ?></a></td>
                    ...
                    <td class="tb_bt"><?php if ( $entrou ) {  ?><div class="dcontexto"><span>Este visitante já está na unidade</span> <img src="<?php if ( empty( $jumbo ) ) {?><?php echo SICOP_SYS_IMG_PATH; ?>visit_in.png<?php } else { ?><?php echo SICOP_SYS_IMG_PATH; ?>visit_jumbo.png<?php } ?>" alt="" class="icon_button" /></div><?php }; ?></td>
                </tr>
                <?php } // fim do while ?>
            </table>
