...
$query_mov_visit = "SELECT
                    ...
                    WHERE
                      visita_mov.cod_visita = $idvisit
                    ORDER BY
                      visita_mov.data_in DESC
                    LIMIT 10";

$q_count_mov_visit = "SELECT
                        COUNT( `idmov_visit` ) AS `total`
                      FROM
                        `visita_mov`
                      WHERE
                        `cod_visita` = $idvisit";
...
$q_det = "SELECT
          ...
          FROM
            detentos
            LEFT JOIN visitas_detentos ON visitas_detentos.cod_detento =  detentos.iddetento
            ...
            LEFT JOIN tipoparentesco ON visitas_detentos.cod_parentesco = tipoparentesco.idparentesco
          WHERE
            visitas_detentos.cod_visita = $idvisit
            AND
...
if ( !$query_obs ) {
    ...
    $msg->add_quebras( 1 );
    $msg->set_msg_pre_def( SM_QUERY_FAIL );
    $msg->add_parenteses( "OBSERVAÇÕES - $motivo_pag" );
    $msg->add_quebras( 2 );
    $msg->set_msg( $msg_err_mysql );
    $msg->get_msg();
...
$cont_obs_destaq = 0;
$cont_obs_destaq = $db->fetchOne( $q_cont_obs );
$db->closeConnection();
...
$query_susp_lista = $db->query( $query_susp_lista );
if ( !$query_susp_lista ) {
    ...
    $msg->add_quebras( 1 );
    $msg->set_msg_pre_def( SM_QUERY_FAIL );
    $msg->add_parenteses( "HISTÓRICO DE SUSPENÇÕES - $motivo_pag" );
    $msg->add_quebras( 2 );
    $msg->set_msg( $msg_err_mysql );
    $msg->get_msg();
...
if ( $query_susp_lista ) {
    $contsusplista = $query_susp_lista->num_rows;
}


$query_mov_visit = $db->query( $query_mov_visit );
if ( !$query_mov_visit ) {
    ...
    $msg->add_quebras( 1 );
    $msg->set_msg_pre_def( SM_QUERY_FAIL );
    $msg->add_parenteses( "HISTÓRICO DE VISITAS - $motivo_pag" );
    $msg->add_quebras( 2 );
    $msg->set_msg( $msg_err_mysql );
    $msg->get_msg();
...
$contmv = 0;
$contmv = $db->fetchOne( $q_count_mov_visit );
$db->closeConnection();
...
if ( !$q_det ) {
    ...
    $msg->add_quebras( 1 );
    $msg->set_msg_pre_def( SM_QUERY_FAIL );
    $msg->add_parenteses( "RELAÇÃO DE DETENTOS - $motivo_pag" );
    $msg->add_quebras( 2 );
    $msg->set_msg( $msg_err_mysql );
    $msg->get_msg();
...
$susp = sitvisit::getInstance();
$susp->get_sit_visit( $idvisit );
//$revog    = $d_visit['revog'];
...
if ( !empty( $proced ) and $proced == 1 ){
    ...
    // executando a query
    $query_det_in = $model->query( $query_det_in );
    ...
    $d_det_in     = $query_det_in->fetch_assoc();
    ...
    if ( $pode_entrar ) {
        ...
        $q_quant_in = "SELECT
                         COUNT( `visitas`.`idvisita` ) AS `total`
                       FROM
                         `visita_mov`
        ...
        // executando a query
        $quant_in = $model->fetchOne( $q_quant_in );
    ...
    if ( $pode_entrar ) {
        ...
        $q_ja_ent  = "SELECT COUNT( `cod_visita` ) AS `total` FROM `visita_mov` WHERE DATE(`data_in`) = DATE(NOW()) AND `cod_visita` = $idv LIMIT 1";

        // instanciando o model
        $model = SicopModel::getInstance();

        // executando a query
        $ja_entrou = $model->fetchOne( $q_ja_ent );
    ...
    if ( $pode_entrar ) {
        ...
        // executando a query
        $quant_jumbo = $model->fetchOne( $q_jumbo );

        // fechando a conexao
        $model->closeConnection();

        if( $quant_jumbo === false ) {
...
// executando a query
(int)$total_in = $model->fetchOne( $q_total_mov_visit );
        ...
        <?php }?>
            ...
            <table class="detal_visit">
                ...
                <tr>
                    ...
                    <td align="center" ><?php if ( $suspenso || $excluido ) { ?> A partir de <?php echo $susp->data_ini ?> <?php } ?></td>
                    <td align="center" ><?php if ( $suspenso ) { ?> Até <?php echo $susp->data_fim ?> <?php } ?></td>
                </tr>
                <?php if ( $suspenso || $excluido ) { ?>
                <tr>
                    <td class="great_mot" colspan="3">Motivo: <?php echo $susp->motivo ?></td>
                </tr>
                <?php } ?>
            ...
            <form action="<?php echo SICOP_ABS_PATH; ?>send/sendvisitin.php" method="post" name="visit_in" id="visit_in" >
                ...
                <table width="660" align="center" class="space">
                    ...
                    <?php while ( $d_out = $q_reg->fetch_assoc() ) { ?>
                    <tr>
            ...
            <div class="linha">
                DETENTOS À QUEM ESTE VISITANTE ESTÁ RELACIONADO <?php if ( $cont_det > 10 ) { ?> - <span id="span_all_visit">Mostrando as 10 últimas entradas - <a id="all_visit_in" href="javascript:void(0)">Mostrar todas</a></span><?php } ?>
                <hr />
            </div>
            <?php
            if( $cont_det < 1 ) { // se o número de ocorrências for menor do que 1, mostra a mensagem
                echo '<p class="p_q_no_result">Nenhum detento.</p>';
            } else {
                $i = 1
                ?>
            <table class="lista_busca">
                ...
                <tr class="even">
                    ...
                    <td class="nome_det_small"><a href="rol_visit.php?iddet=<?php echo $dados['iddetento'];?>" title="Pai: <?php echo $dados['pai_det'];?>&#13;Mãe: <?php echo $dados['mae_det'];?>&#13;Situação atual: <?php echo $det['sitat']; ?>"><?php echo $dados['nome_det'];?></a></td>
                    <td class="matr_det <?php echo $det['css_class']; ?>"><?php if ( !empty( $dados['matricula'] ) ) echo formata_num( $dados['matricula'] ); else echo '&nbsp;'; ?></td>
                    <td class="raio_det <?php echo $det['css_class']; ?>"><?php echo !empty( $dados['raio'] ) ? $dados['raio'] : '&nbsp;'; ?></td>
                    <td class="cela_det <?php echo $det['css_class']; ?>"><?php echo !empty( $dados['cela'] ) ? $dados['cela'] : '&nbsp;'; ?></td>
                    <td class="visit_parent <?php echo $det['css_class']; ?>"><?php echo $dados['parentesco'] ?></td>
                    <td class="tb_bt"><?php if ( $n_rol >= 3 ) {  ?> <a href="editsuspvisit.php?idsusp=<?php echo '' ?>" title="Alterar esta suspenção" ><img src="<?php echo $img_sys_path; ?>b_edit.png" alt="Alterar este relacionamento" /></a> <?php }; ?> </td>
                    <td class="tb_bt"><?php if ( $n_rol >= 4 ) {  ?> <a href='javascript:void(0)' onclick='drop_susp_visit(<?php echo $idvisit; ?>, <?php echo '' ?>)' title="Excluir esta suspenção" ><img src="<?php echo $img_sys_path; ?>b_drop.png" alt="Excluir este relacionamento" class="icon_button" /></a> <?php }; ?> </td>
                    <?php } // fim do while ?>
                </tr>
            </table>
            ...
            <div class="linha">
                HISTÓRICO DE VISTITAS <?php if ( $contmv > 10 ) { ?> - <span id="span_all_visit">Mostrando as 10 últimas entradas - <a id="all_visit_in" href="javascript:void(0)">Mostrar todas</a></span><?php } ?>
                <hr />
            </div>
            <?php
            if( $contmv < 1 ) { // se o número de ocorrências for menor do que 1, mostra a mensagem
                ...
                <div class="grid_div" id="hist_visit">

                    <?php while( $d_mov_v = $query_mov_visit->fetch_assoc() ) { ?>
            ...
            <table class="lista_busca">
                <tr >
                    ...
                    <?php
                    while( $d_susp = $query_susp_lista->fetch_assoc() ) {
