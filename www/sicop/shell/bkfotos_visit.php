...
$command1 = 'smbmount //' . SICOP_BACKUP_IP . '/bkfotos_visit /mnt -o username=' . SICOP_BACKUP_USER . ',password=' . SICOP_BACKUP_PASS;

system( $command1, $return_cm1 );

// se o diretório foi montado... ( se $command1 retornou 0 - nenhum erro )
if ( empty( $return_cm1 ) ) {

    // comando para copiar o os arquivos da pasta do servidor para a pasta de backup ( somente novos arquivos )
    $command2 = 'cp -u ' . SICOP_VISIT_FOLDER . '* /mnt/';
...
$msg->add_chaves( 'BACKUP DE FOTOS DE VISITANTES', 0, 1 );
$msg->set_msg( 'Backup de fotos de visitantes realizada com sucesso.' );

if ( !empty( $return_cm1 ) or  !empty( $return_cm2 ) or !empty( $return_cm3 ) ) {

    $msg->set_msg_type( SM_TYPE_ERR );
    $msg->add_quebras( 1 );
    $msg->set_msg( 'Falha ao realizar backup de fotos de visitantes.' );
