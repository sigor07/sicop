...
if ( $n_msg < $n_msg_n ) {
    ...
    $msg = array();
    $msg['tipo']     = 'perm';
    $msg['entre_ch'] = 'MENSAGEM - RECEBIDA';
    get_msg( $msg, 1 );
...
if ( $is_post ) {
    ...
    $query_msg = "UPDATE `msg` SET `msg_para_exc` = 1, `msg_para_exdata` = NOW() WHERE `idmsg` = $idmsg AND `msg_para` = $iduser LIMIT 1";
...
$q_msg_up = "UPDATE `msg` SET `msg_para_ultdata` = NOW() WHERE `idmsg` = $idmsg LIMIT 1";

if ( $lida == 0 ) {
    $q_msg_up = "UPDATE `msg` SET `msg_para_lida` = 1, `msg_para_vdata` = NOW(), `msg_para_ultdata` = NOW() WHERE `idmsg` = $idmsg LIMIT 1";
}
...
?>
        <script type="text/javascript" >
            ...
            <p class="descript_page">MENSAGEM</p>
            ...
            <table class="detal_msg">
                ...
                <tr>
                    <td class="desc_user" ><?php if ( $d_msg['msg_para_vdata'] and $d_msg['msg_para_ultdata'] ) { ?> Lida em em <?php echo $d_msg['msg_para_vdata'] ?>, última visualização em <?php echo $d_msg['msg_para_ultdata'] ?> <?php } ?></td>
                </tr>
            </table>
            ...
            <?php } ?>
                <div class="form_bts">
                    ...
                    <?php if ( $n_msg >= 3 ) { ?>
                    <input class="form_bt" name="" type="button" onclick="javascript: location.href='rmsg.php?idmsg=<?php echo $d_msg['idmsg']; ?>';" value="Responder" />
