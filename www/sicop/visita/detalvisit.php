...
if ( empty( $idvisit ) ) {
    ...
    $msg = array();
    $msg['tipo']  = 'atn';
    $msg['text']  = "Tentativa de acesso direto à página. Identificador do visitante em branco. ( DETALHES DO VISITANTE )";
    get_msg( $msg, 1 );
...
/*
$query_susp_limit = $query_susp . "WHERE
                                     ( `cod_visita` = $idvisit AND ( ( CURDATE() BETWEEN `data_inicio` AND ADDDATE(`data_inicio`, `periodo`) ) OR ( CURDATE() >= `data_inicio` AND ISNULL( ADDDATE(`data_inicio`, `periodo`) ) ) ) AND `revog` = FALSE )
                                   ORDER BY ADDDATE(`data_inicio`, `periodo`) ASC
                                   LIMIT 1";
 *
 */

//depur($query_susp_lista);
...
if ( !$query_visit ) {
    ...
    $msg = array();
    $msg['tipo']  = 'err';
    $msg['text']  = 'Falha na consulta ( DETALHES DO VISITANTE ).';
    $msg['linha'] = __LINE__;
    get_msg( $msg, 1 );
...
if ( $contv < 1 ) {
    ...
    $msg = array();
    $msg['tipo']  = 'err';
    $msg['text']  = 'A consulta retornou 0 ocorrências ( DETALHES DO VISITANTE ).';
    $msg['linha'] = __LINE__;
    get_msg( $msg, 1 );
...
(int)$cont_obs_destaq = $model->fetchOne( $q_cont_obs );
...
if ( empty( $iddet ) ){
    ...
    $msg = array();
    $msg['tipo']  = 'err';
    $msg['text']  = 'Identificador d' . SICOP_DET_ART_L . ' ' . SICOP_DET_DESC_L . ' no visitante em branco ( DETALHES DO VISITANTE ).';
    $msg['linha'] = __LINE__;
    get_msg( $msg, 1 );
...
$nome_det = SICOP_DET_DESC_FU . ': <a href="' . SICOP_ABS_PATH . 'detento/detalhesdet.php?iddet='.$d_det['iddetento'].'" title="Clique aqui para abrir a qualificativa d' . SICOP_DET_PRON_L . SICOP_DET_DESC_L .'">'.$d_det['nome_det'].'</a>';
$matr     = 'Matrícula: ' . formata_num($d_det['matricula']);
$raio     = SICOP_RAIO . ': '.$d_det['raio'];
$cela     = SICOP_CELA . ': '.$d_det['cela'];
...
// executando a query
$query_susp_lista = $model->query( $query_susp_lista );

// fechando a conexao
$model->closeConnection();

$contsusplista = 0;
if ( $query_susp_lista ) $contsusplista = $query_susp_lista->num_rows;

$susp = manipula_sit_visia_cq( $idvisit );
...
?>
        ...
        <?php }?>
            ...
            <p class="link_common">
                ...
                <?php if ( $n_rol >= 4 ) { ?> | <a href='javascript:void(0)' onclick='drop_visit(<?php echo $iddet; ?>, <?php echo $d_visit['idvisita']; ?>)' title="Excluir este visitante">Excluir Visitante</a><?php }; ?>
            </p>
            <?php }; ?>

            <p class="link_common">
                <a href="rol_visit.php?iddet=<?php echo $d_det['iddetento'] ?>" title="Ir para o rol d<?php echo SICOP_DET_ART_L . ' ' . SICOP_DET_DESC_L; ?>">Ir para o rol</a>
            </p>
            ...
            <table class="detal_visit">
                ...
                <tr>
                    <td class="mid">Parentesco: <?php echo $d_visit['parentesco'] ?></td>
                ...
                <?php } ?>
                <tr>
                    <td class="mid_det"><?php echo $nome_det ?></td>
                    <td class="mini_det"><?php echo $matr ?></td>
                    <td class="mini_det_destaque"><?php echo $raio ?> - <?php echo $cela ?></td>
                </tr>
                <tr>
            ...
            <form action="<?php echo SICOP_ABS_PATH; ?>send/sendvisitin.php" method="post" name="visit_in" id="visit_in" >
                ...
                <table width="660" align="center" class="space">
                    ...
                    <tr>
                        ...
                        <td class="oculta"><input type="hidden" name="idvisit[]" value="<?php echo $d_out['cod_visita'];?>" /></td>
                    </tr>
            ...
            // executando a query
            $query_mov_visit = $model->query( $query_mov_visit );
            ...
            if( $query_mov_visit ) $contmv = $query_mov_visit->num_rows;
