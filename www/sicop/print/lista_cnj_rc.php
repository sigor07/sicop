...
if ( !$is_post ) {
    $mensagem = "<span class='desc_atencao'>*** ATENÇÃO ***</span> -> Tentativa de acesso direto à página de impressão de resultado de consulta.<br /><br /> Página: $pag";
    salvaLog( $mensagem );
    echo msg_js( '', 'f' );
    exit;
}
...
$q_lista = "SELECT DISTINCT
              ...
              `raio`.`raio`,
              `mov_rc_det`.`data_rc`,
              IF( `mov_rc_det`.`data_rc`, DATE_FORMAT( `mov_rc_det`.`data_rc`, '%d/%m/%Y' ), DATE_FORMAT( `mov_det_in`.`data_mov`, '%d/%m/%Y' ) ) AS `data_rc_f`
            FROM
              `detentos`
              ...
              LEFT JOIN `mov_rc_det` ON `detentos`.`iddetento` = `mov_rc_det`.`cod_detento`
            WHERE
              ...
              AND
              ( `mov_rc_det`.`id_mov_rc` = ( SELECT MAX( `mov_rc_det`.`id_mov_rc` ) FROM `mov_rc_det` WHERE `mov_rc_det`.`cod_detento` = `detentos`.`iddetento` )
                 OR
                 ISNULL( `mov_rc_det`.`data_rc` )
              )
            ORDER BY
              `raio`.`raio` ASC, `cela`.`cela` ASC, `detentos`.`nome_det` ASC";
...
$motivo = 'LISTA DE ' . SICOP_DET_DESC_U;
if ( !empty( $desc_pag ) ) {
    $motivo = $desc_pag;
}
        ...
        <div class="corpo">
            ...
            <table width="650" align="center" class="bordasimples_f" border="1" cellpadding="1" cellspacing="0">
                <thead>
                    <tr>
                        ...
                        <th scope="col" width="100" align="center">Inclusão</th>
                        <th scope="col" width="125" align="center">Inclusão na cela</th>
                    </tr>
                </thead>
                <?php
                ...
                <tbody>
                    <tr>
                        ...
                        <td align="center" class="espaco_td"><?php echo $d_det['data_incl_f']?></td>
                        <td align="center" class="espaco_td"><?php echo $d_det['data_rc_f']?></td>
                    </tr>
                </tbody>
            ...
            <table width="650" align="center" class="bordasimples_f" border="1" cellpadding="1" cellspacing="0">
                <thead>
                    <tr>
                        <th scope="col" width="40" height="15" align="center">N</th>
                        <th scope="col" width="230" align="center">Detento</th>
                        <th scope="col" width="75" align="center">Matrícula</th>
                        <th scope="col" width="30" align="center"><?php echo SICOP_RAIO_AB; ?></th>
                        <th scope="col" width="20" align="center"><?php echo SICOP_CELA_AB; ?></th>
                        <th scope="col" width="100" align="center">Inclusão</th>
