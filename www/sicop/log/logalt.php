...
if ( isset( $_GET['limit'] ) and $_GET['limit'] == 'yes' ) {
    ...
    $query_log = "SELECT
                  ...
                  `log_alt`.`alteracao`,
                  `log_alt`.`data`,
                  DATE_FORMAT( `log_alt`.`data`, '%d/%m/%Y às %H:%i:%s' ) AS data_f,
                  `log_alt`.`user`,
                  `log_alt`.`ip`,
                  `sicop_users`.`nome_cham`
                FROM
                  `log_alt`
                  LEFT JOIN `sicop_users` ON `log_alt`.`user` = `sicop_users`.`iduser`
                ORDER BY
                  `log_alt`.`data` DESC
                $limit";
    ...
    $sql = 'SELECT COUNT(*) AS total FROM `log_alt`';
...
?>
            ...
            <p class="descript_page">LOG DE REGISTRO DE ALTERAÇÕES</p>
            ...
            <p class="link_common">Ocorrências com mais de 30 dias serão automaticamente excluidas.<br />
                ...
                <?php if ( isset( $_GET['limit'] ) and $_GET['limit'] == 'yes' ) { ?>
                    Mostrando as 1000 últimas ocorrencias. <a href="logalt.php" >Mostrar todas</a>
                <?php } else { ?>
                    Mostrando todas as alterações registradas. <a href="logalt.php?limit=yes" >Mostrar apenas as 1000 últimas ocorrencias.</a>
                <?php } ?>
            ...
            <table class="lista_log">
                ...
                <tr>
                    ...
                    <th class="log_tipo" scope="col">Tipo</th>
                    <th class="log_tab" scope="col">Tabela</th>
                    <th class="log_msg_min" scope="col">Mensagem</th>
                ...
                <tr class="even">
                    <td class="log_user"><a href="<?php echo SICOP_ABS_PATH ?>user/user.php?iduser=<?php echo $log['user'] ?>"><?php echo $log['nome_cham'] ?></a></td>
                    <td class="log_data"><?php echo $log['data_f'] ?></td>
                    <td class="log_tipo"><?php echo $log['tipo_log'] ?></td>
                    <td class="log_tab"><?php echo $log['tabela'] ?></td>
                    <td class="log_msg_min"><?php echo nl2br( $log['alteracao'] ) ?></td>
