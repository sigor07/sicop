...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query_in = "INSERT INTO `numerorms`
                            (`numero_rms`, `ano`, `iduser`, `idsetor`, `coment`)
                            VALUES";

    $valores = "";

    for ($i=0; $i < $quant; ++$i) {
        $valores .= " ( (SELECT IFNULL(MAX(numrms.numero_rms), 0) FROM numerorms numrms WHERE ano = YEAR(NOW())) + 1, YEAR(NOW()), $iduser, $idsetor, $coment),";
    }
    ...
    $_SESSION['num_rms'] = implode(', ', $id);

    header("Location: numrmsok.php");
    exit;
...
$trail->add('Solicitar números de remessa', $_SERVER['PHP_SELF'], 3);
$trail->output();
?>

            <p class="descript_page">SOLICITAR NÚMERO(S) PARA REMESSA</p>

            <form action="numrms.php" method="post" name="numrms" id="numrms" onSubmit="return validanum();">
