...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query_in = "INSERT INTO `numeronotes`
                            (`numero_notes`, `ano`, `iduser`, `idsetor`, `coment`)
                            VALUES";

    $valores = "";

    for ($i=0; $i < $quant; ++$i) {
        $valores .= " ( (SELECT IFNULL(MAX(numnotes.numero_notes), 0) FROM numeronotes numnotes WHERE ano = YEAR(NOW())) + 1, YEAR(NOW()), $iduser, $idsetor, $coment ),";
    }
    ...
    $_SESSION['num_notes'] = implode( ', ', $id );

    header( 'Location: numnotesok.php' );
    exit;
...
$trail->add('Solicitar números de notes', $_SERVER['PHP_SELF'], 3);
$trail->output();
?>


            <p class="descript_page">SOLICITAR NÚMERO(S) PARA NOTES</p>

            <form action="numnotes.php" method="post" name="numnotes" id="numnotes" onSubmit="return validanum();">
